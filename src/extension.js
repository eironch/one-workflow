  ...
  _handleAutomation(enabled) {
    ...
    if (enabled) {
      this._interval = setInterval(() => {
        ...
        const command = `$wshell = New-Object -ComObject WScript.Shell; $targets = @(${IDEs.map(i => `'${i}'`).join(',')}); foreach ($t in $targets) { if ($wshell.AppActivate($t)) { $wshell.SendKeys('${settings.automationKey}') } }`;
